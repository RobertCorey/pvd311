...
- [x] **Accounts required** — live 2026-08-22: gate at Send (draft preserved), Worker `401 auth_required`, identity from the token, uid pacing, per-account ramp (N=3, HITL_MODE=ramp), sign-in links minted by the Worker via Resend; Google sign-in verified live (OAuth origins set by bob).
