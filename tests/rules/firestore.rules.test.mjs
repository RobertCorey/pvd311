...
await gap('unknown field denied', () => assertFails(create(anon, good('anon-1', { approvedAt: 'x' }))));
await gap('out-of-bbox denied', () => assertFails(create(anon, good('anon-1', { lat: 42.36, lng: -71.06 }))));
await t('null lat/lng ok (manual address)', () => assertSucceeds(create(anon, good('anon-1', { lat: null, lng: null }))));
await gap('half-null lat/lng denied', () => assertFails(create(anon, good('anon-1', { lat: null }))));
await gap('non-storage photo URL denied', () => assertFails(create(anon, good('anon-1', { photo: 'https://evil.example/x.jpg' }))));
...
await gap('data: URL photo denied', () => assertFails(create(anon, good('anon-1', { photo: 'data:image/jpeg;base64,AAAA' }))));
await t('long description denied', () => assertFails(create(anon, good('anon-1', { description: 'x'.repeat(2001) }))));
await gap('short address denied', () => assertFails(create(anon, good('anon-1', { address: 'ab' }))));
