...
class WatcherBrowser {
  ...
  private getPage(): Page {
    if (!this.page) throw new Error('Watcher browser not launched');
    return this.page;
  }
   ...
   */
  async scrapeMyRequests(): Promise<Map<string, PortalRow>> {
    const page = this.getPage();
    ...
    for (let p = 0; p < MAX_PAGES; p++) {
      ...
      for (const row of rows) {
        ...
        if (parsed && !byId.has(parsed.caseId)) byId.set(parsed.caseId, parsed);
      }
  ...
  private async gotoNextGridPage(): Promise<boolean> {
    ...
    if (await next.evaluate((el) => el.closest('li')?.classList.contains('disabled') ?? false).catch(() => false)) return false;
   ...
   */
  async fetchLatestActivity(caseId: string): Promise<PortalActivity | null> {
    const page = this.getPage();
    let captured: string | null = null;
    const onResponse = (resp: { url(): string; text(): Promise<string> }) => {
    ...
    try {
      ...
      for (let i = 0; i < 20 && captured === null; i++) await page.waitForTimeout(400);
...
export function parseRow(headers: string[], cells: string[]): PortalRow | null {
  ...
  const street = iStreet >= 0 && cells[iStreet] ? cells[iStreet].trim()
    : (cells.find((c, i) =>
        i !== iTitle && c && !CASE_ID_RE.test(c) && !isStatusLike(c) && !looksLikeDate(c)
...
export function parseActivities(body: string): { subject: string; createdOn: string | null } | null {
  let data: unknown;
  ...
  let best: { subject: string; createdOn: string | null; t: number } | null = null;
  for (const raw of items) {
    ...
    if (!best || score > best.t) best = { subject, createdOn, t: score };
  }
  return best ? { subject: best.subject, createdOn: best.createdOn } : null;
}

function findActivityArray(data: unknown): unknown[] {
  if (Array.isArray(data)) return data;
  if (data && typeof data === 'object') {
    const o = data as Record<string, unknown>;
    // OData shape: { d: { results: [...] } } or { d: [...] }
    if (o['d'] !== undefined) return findActivityArray(o['d']);
 ...
 */
export async function runWatcher(): Promise<Change[]> {
  const db = getDb();
  if (!db) throw new Error('Firestore not initialised — call initFirestore() before runWatcher()');
  ...
  const tracked = snap.docs
    .map((d) => ({ ...(d.data() as Report & { portalStatus?: string }), id: d.id }))
  ...
  try {
    ...
    for (const report of tracked) {
      const caseId = report.portalCaseId as string;
    ...
    if (changes.length) invalidateCache();
  } finally {
    await browser.close();
  }
