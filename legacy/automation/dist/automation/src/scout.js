...
export async function scoutFields(input) {
    if (!process.env['ANTHROPIC_API_KEY']) {
        throw new Error(`NEEDS_MAPPING: ${input.controls.length} unmapped control(s) for "${input.caseTypeName}" ` +
    ...
    if (!block)
        throw new Error(`Scout returned no tool call (stop_reason=${response.stop_reason})`);
