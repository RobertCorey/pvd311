...
const ICON = {
  map: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M3 6l6-3 6 3 6-3v15l-6 3-6-3-6 3z" /><path d="M9 3v15M15 6v15" /></svg>,
...
/** Phone bottom tabs: Map · Report (ember, center) · Mine. Hidden by CSS while a report is in progress. */
export default function TabBar() {
  const t = useT();
  return (
    <nav className="tabbar" aria-label={t('nav.tabs')}>
      <div className="tabbar-inner">
        <NavLink to="/map" className="tab">{ICON.map}<span>{t('nav.map')}</span></NavLink>
